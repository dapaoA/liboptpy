...
class ConstantInvIterStepSize(StepSize):
    def __init__(self,stepsize):
        self.stepsize = stepsize
    
    def get_stepsize(self, h, x, num_iter, *args):
        return self.stepsize / num_iter
...
class Dynamic_Backtracking(StepSize):
    '''
    Class represents different rules for backtracking search of step size
    '''
    ...
    def get_stepsize(self, h, x, num_iter, *args):
        ...
        if self.rule == "Armijo":
            ...
            current_grad = self._grad(x)
            current_f = self._f(x)
            x_next = self._update_x_next(x, alpha, h,*args)
            while True:
                if np.isnan(self._f(x_next)):
                    alpha *= rho
                else:
                ...
                if alpha < 1e-16:
                    raise ValueError("Step size is too small!")
                x_next = self._update_x_next(x, alpha, h,*args)
    #            self.par["init_alpha"] = alpha
            return alpha, x_next
        elif self.rule == "Armijo_pgd":
            rho = self.par["rho"]
            beta = self.par["beta"]
            assert beta < 0.5, "Armijo rule is applicable for beta less than 0.5"
            assert rho < 1, "Decay factor has to be less than 1"
            ...
            while True:
                if np.isnan(self._f(x_next)):
                    ...
                    if self._f(x_next) >= current_f + beta * current_grad.dot(x_next - x) + beta * np.dot((x_next - x).T,x_next - x) :
                        alpha *= rho
                    else:
                        break
                if alpha < 1e-16:
    ...
    #            self.par["init_alpha"] = alpha
            ...
            while True:
                ...
                if np.isnan(self._f(x_next)):
                    ...
                    if self._f(x_next) > current_f + current_grad.dot(x_next - x) + np.linalg.norm(x_next - x) ** 2 / (
                            2 * self._alpha) + eps:
                        self._alpha *= rho
                    else:
                        if self.par["disp"]:
            ...
            raise NotImplementedError(
                "Available rules for backtracking are 'Armijo', 'Goldstein', 'Wolfe', 'Wolfe strong' and 'Lipschitz'")
...
class Backtracking_Nestrov(StepSize):
    '''
    Class represents different rules for backtracking search of step size
    '''
    ...
    def assign_function(self, f, grad, update_x_next, update_y_next):
        self._f = f
        self._grad = grad
        self._update_x_next = update_x_next
        self._update_y_next = update_y_next

    def get_stepsize(self, h, y, num_iter,tk, *args):
        alpha = self.par["init_alpha"]
        if self.rule == "Armijo":
            rho = self.par["rho"]
            beta = self.par["beta"]
            assert beta < 0.5, "Armijo rule is applicable for beta less than 0.5"
            assert rho < 1, "Decay factor has to be less than 1"
            current_grad = self._grad(y)
            current_f = self._f(y)
            x_next = self._update_x_next(y, alpha, h)
            y_next = self._update_y_next(x_next,tk)
            while True:
                if np.isnan(self._f(y_next)):
                    alpha *= rho
                else:
                    if self._f(y_next) >= current_f + beta * current_grad.dot(y_next - y):
                        alpha *= rho
                    else:
                        break
                if alpha < 1e-32:
...
class Backtracking_Bregman_Nestrov(StepSize):
    '''
    Class represents different rules for backtracking search of step size
    '''
        ...
        if self.rule == "Armijo":
            ...
            while True:
                if np.isnan(self._f(y_next)):
                    ...
                    if self._f(y_next) >= current_f + beta * (current_grad.dot(y_next - y)+ 1/alpha * self.bregman(y_next,y)):
                        alpha *= rho
                    else:
                        break
                if alpha < 1e-64:
    ...
    def bregman(self,x, y):
        return np.sum(np.dot(x, np.log(x)-np.log(y)) - x + y)


class D_Backtracking_Nestrov(StepSize):
    '''
    Class represents different rules for backtracking search of step size
    '''
