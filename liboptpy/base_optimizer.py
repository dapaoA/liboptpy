...
class Coor_Optimizer(object):
    def __init__(self, f, mini, dim_a,batch=1, memory_size=1, **kwargs):
        self.convergence = []
        self.time = []
        self._f = f
        self._mini = min
    ...
    def solve(self, x0, max_iter=100, tol=1e-6, disp=False):
        ...
        while True:
            self._id = np.random.choice(ids,self._batch,replace=False)
            self._h = self.get_coord(self._x_current, self._id) #这种就没有了吧，直接是一步到位
            if disp > 1:
                print("Iteration {}/{}".format(iteration, max_iter))
                print("Current function val =", self._f(self._x_current))
                self._print_info()
    ...
    def _update_x_next(self):
        self._x_next = self._f_update_x_next(self._x_current,self._id)
