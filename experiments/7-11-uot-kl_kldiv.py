...
from functions import uot_kl_proximal_K_entropy as p
...
grad = lambda x: g(x, a, b, m, tau, HcHc,HrHr,Hcb,Hra, dim_a,dim_b)


proximal = lambda x,h,grad: p(m,x,h,grad)
...
"MD": cs.MirrorD_gen(f, grad, proximal, ss.ConstantStepSize(0.01)),
...
plt.imshow(Gs, cmap='hot', interpolation='nearest')
plt.title('uot')
plt.show()
# time_s = time.time()
