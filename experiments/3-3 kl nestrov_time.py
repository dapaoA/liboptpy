...
for i in range(Ex_times):
    ...
    time_s = time.time()
    G1_q000005, log_q000005 = sot.mm_unbalanced_dynamic2_stop(a, b, M,0.1,tau,0.0001,2, div='kl',numItermax=round,log=True,stopThr=stopThr)
    time_e = time.time()
    print("time costs: ", time_e - time_s, " s")
    time_s = time.time()
    ...
    time_s = time.time()
    G1_q000005a2, log_q000005a2 = sot.mm_unbalanced_dynamic2_stop_nestrov2(a, b, M,0.1,tau,0.0001,2, div='kl',numItermax=round,log=True,stopThr=stopThr)
    time_e = time.time()
    print("time costs: ", time_e - time_s, " s")
    time_s = time.time()
...
for con, c in zip(convergence, colors):
    ...
    for j in range(Ex_times):
        error[con][j, :] = np.asarray([np.log(f(x, a_list[j], b_list[j], M_list[j]) - opt_list[j])
...
axs.set_ylabel(r'$\ln(\textbf{P}_{\text{UOT}}(\textbf{T}^{k}) - \textbf{P}_{\text{OT}}(\textbf{T}^{*}))$')
axs.set_xlim(0, 100)
