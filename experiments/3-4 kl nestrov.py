...
for i in range(Ex_times):
    ...
    time_s = time.time()
    G1_q000005, log_q000005 = ot.unbalanced.mm_unbalanced_dynamic2_stop(a, b, M,0.1,tau,0.0001,2, div='kl',numItermax=round,log=True,stopThr=stopThr)
    time_e = time.time()
    print("time costs: ", time_e - time_s, " s")
    time_s = time.time()
    ...
    time_s = time.time()
    G1_q000005a2, log_q000005a2 = ot.unbalanced.mm_unbalanced_dynamic2_stop_nestrov2(a, b, M,0.1,tau,0.0001,2, div='kl',numItermax=round,log=True,stopThr=stopThr)
    time_e = time.time()
    print("time costs: ", time_e - time_s, " s")
    time_s = time.time()
...
for con, c in zip(convergence, colors):
    error[con] = np.zeros((Ex_times, len(convergence[con][0]['G'][::paint_iteration])))
    for j in range(Ex_times):
        error[con][j, :] = np.asarray([(f(x, a_list[j], b_list[j], M_list[j]))
                                       for x in convergence[con][j]['G'][::paint_iteration]])
