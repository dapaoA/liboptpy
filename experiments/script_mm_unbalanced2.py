...
def sinkhorn_unbalanced_unequal(a, b, M, reg, reg_ma,reg_mb, method='sinkhorn', numItermax=1000,
                        stopThr=1e-6, verbose=False, log=False, **kwargs):
    r"""
    Solve the unbalanced entropic regularization optimal transport problem
    and return the OT plan
    ...
    if method.lower() == 'sinkhorn':
        return sinkhorn_knopp_unbalanced_unequal(a, b, M, reg, reg_ma,reg_mb,
                                         numItermax=numItermax,
                                         stopThr=stopThr, verbose=verbose,
                                         log=log, **kwargs)
    else:
...
def sinkhorn_knopp_unbalanced_unequal(a, b, M, reg, reg_ma,reg_mb, numItermax=1000,
                              stopThr=1e-6, verbose=False, log=False, **kwargs):
    r"""
    Solve the entropic regularization unbalanced optimal transport problem and return the loss
...
def mm_unbalanced(a, b, M, reg_m, div='kl', G0=None, numItermax=1000,
    ...
    for i in range(numItermax):
        ...
        if div == 'kl':
            ...
            G = nx.maximum(G-reg_m*(nx.tile(nx.sum(G, 0, keepdims=True),(a.shape[0],1)) + nx.tile(nx.sum(G, 1, keepdims=True),(1,b.shape[0]))-K),0)
...
def mm_unbalanced_revised_nestrov(a, b, M, reg_m, l_rate=0.5, div='kl', G0=None, numItermax=1000,
    ...
    for i in range(numItermax):
        ...
        if div == 'kl':
            ...
            Gd = nx.sum(G, 0, keepdims=True) + nx.sum(G, 1, keepdims=True) + 1e-16
            G = G * K / Gd
            G  = nx.maximum(G + ((tk - 1) / t_knew) * (G - Gprev),0)
        elif div == 'l2_2':
            G = nx.maximum(G-l_rate*(nx.tile(nx.sum(G, 0, keepdims=True),(a.shape[0],1)) + nx.tile(nx.sum(G, 1, keepdims=True),(1,b.shape[0]))-K),0)
            G  = nx.maximum(G + ((tk - 1) / t_knew) * (G - Gprev),0)
...
def mm_unbalanced_dynamic(a, b, M, sreg_m,ereg_m,step, div='kl', G0=None,l_rate=0.01, numItermax=1000,
                  stopThr=1e-15, verbose=False, log=False):
    r"""
    Solve the unbalanced optimal transport problem and return the OT plan.
    The function solves the following optimization problem:
    ...
    for i in range(numItermax):
        ...
        if div == 'kl':
            if(i<=step):
                localreg = ereg_m * (i)/step + sreg_m*(1-(i)/step)
...
def mm_unbalanced_dynamic2(a, b, M, sreg_m,ereg_m,step, mult,div='kl', G0=None, numItermax=1000,
                  stopThr=1e-15, verbose=False, log=False,stoping_condition = 'normal'):
    r"""
...
def mm_unbalanced_dynamic3(a, b, M, sreg_m,ereg_m,step,div='kl',l_rate=0.01, G0=None, numItermax=1000,
                  stopThr=1e-15, verbose=False, log=False):
    r"""
    use double method to increase
    ...
    if div == 'kl':
        ...
        K = nx.maximum(a[:, None] + b[None, :] - M / sreg_m / 2,
                       nx.zeros((dim_a, dim_b), type_as=M))
    elif div == 'l2_2':
    ...
    for i in range(numItermax):
        ...
        if div == 'kl':
            if (i <= step):
                localreg = nx.exp((i / step * (nx.log(ereg_m)) + (step - i) / step * (nx.log(sreg_m))))
                K = nx.exp(M / -  localreg / 2)
            u = nx.sqrt(a / (nx.sum(G, 1) + 1e-16))
            v = nx.sqrt(b / (nx.sum(G, 0) + 1e-16))
            G = G * K * u[:, None] * v[None, :]
            ...
            if (i <= step):
                localreg = nx.exp((i / step * (nx.log(ereg_m)) + (step - i) / step * (nx.log(sreg_m))))
                K = nx.maximum(a[:, None] + b[None, :] - M / localreg / 2,
                               nx.zeros((dim_a, dim_b), type_as=M))
            Gd = nx.sum(G, 0, keepdims=True) + nx.sum(G, 1, keepdims=True) + 1e-16
            G = G * K / Gd
        elif div == 'l2_2':
...
def mm_unbalanced_dynamic3_nestrov(a, b, M, sreg_m,ereg_m,step,div='kl', l_rate=0.01,G0=None, numItermax=1000,
                  stopThr=1e-15, verbose=False, log=False):
    r"""
    use double method to increase
...
def mm_unbalanced_dynamic2_stop(a, b, M, sreg_m,ereg_m,Q_para, mult,div='kl', G0=None, numItermax=1000,
                  stopThr=1e-15, verbose=False, log=False,stoping_condition = 'normal'):
    r"""
    use double method to increase
    ...
    for i in range(numItermax):
        ...
        err = nx.sqrt(nx.sum((G - Gprev) ** 2))
        if (localreg < ereg_m):
            if (err < Q_para / localreg):
                localreg = min(ereg_m, localreg * mult)
                if div == 'kl':
                    K = nx.exp(M / - (localreg) / 2)
                elif div == 'l2':
                    K = nx.maximum(a[:, None] + b[None, :] - M / localreg / 2,
...
def mm_unbalanced_dynamic2_stop_nestrov(a, b, M, sreg_m,ereg_m,Q_para, mult,div='kl', G0=None, numItermax=1000,
                  stopThr=1e-15, verbose=False, log=False,stoping_condition = 'normal'):
    r"""
    use double method to increase
...
def mm_unbalanced_dynamic2_stop_nestrov2(a, b, M, sreg_m,ereg_m,Q_para, mult,div='kl', G0=None, numItermax=1000,
                  stopThr=1e-15, verbose=False, log=False,stoping_condition = 'normal'):
    r"""
    use double method to increase
...
def mm_unbalanced_dynamic2_stop_start(a, b, M, sreg_m,ereg_m,Q_para, mult,div='kl', G0=None, numItermax=1000,
                  stopThr=1e-15, verbose=False, log=False,stoping_condition = 'normal'):
    r"""
    use double method to increase
    ...
    for i in range(numItermax):
        ...
        if (localreg < ereg_m):
            if (err < Q_para / localreg):
                localreg = max(ereg_m, localreg * mult)
                K = nx.exp(M / - (localreg) / 2)
                print(i,":double:",localreg)
        if log:
            log['err'].append(err)
...
def mm_unbalanced_revised_unequaltau(a, b, M, reg_mb,reg_ma, l_rate=0.5, div='kl', G0=None, numItermax=1000,
                  stopThr=1e-15, verbose=False, log=False):
    r"""
    Solve the unbalanced optimal transport problem and return the OT plan.
    The function solves the following optimization problem:
    ...
    if(reg_mb>=reg_ma):
        ra = reg_ma/(reg_ma+reg_mb)
        rb = reg_mb/(reg_ma+reg_mb)
        l_rate = reg_ma/(reg_ma+reg_mb)
    if div == 'kl':
        K = nx.exp(M / - (reg_ma /l_rate))
    elif div == 'l2':
        K = nx.maximum(a[:, None] + b[None, :] - M / reg_m / 2,
                       nx.zeros((dim_a, dim_b), type_as=M))
    elif div == 'l2_2':
    ...
    for i in range(numItermax):
        ...
        if div == 'kl':
            u = nx.power(a / (nx.sum(G, 1) + 1e-16),ra)
            v = nx.power(b / (nx.sum(G, 0) + 1e-16),rb)
            G = G * K * u[:, None] * v[None, :]
        elif div == 'l2':
            Gd = nx.sum(G, 0, keepdims=True) + nx.sum(G, 1, keepdims=True) + 1e-16
            G = G * K / Gd
...
def mm_unbalanced_revised_coord(a, b, M, reg_m,batch, l_rate=0.5, div='kl', G0=None, numItermax=1000,
                  stopThr=1e-15, verbose=False, log=False):
    r"""
    Solve the unbalanced optimal transport problem and return the OT plan.
    The function solves the following optimization problem:
    ...
    for i in range(numItermax):
        ...
        if div == 'kl':
            arr = nx.arange(dim_a)
            nx.shuffle(arr)
            for j in range(math.floor(dim_a/batch)):
                u = nx.power(a[arr[(j)*batch:(j+1)*batch]] / (nx.sum(G[arr[(j)*batch:(j+1)*batch],:], 1) + 1e-16), l_rate)
                G[arr[(j) * batch:(j + 1) * batch], :] *= (K[arr[(j) * batch:(j + 1) * batch], :].T * u).T
                v = nx.power(b[arr[(j)*batch:(j+1)*batch]] / (nx.sum(G[:,arr[(j)*batch:(j+1)*batch]], 0) + 1e-16), l_rate)
                G[:,arr[(j)*batch:(j+1)*batch]] *= K[:,arr[(j)*batch:(j+1)*batch]]*v
                G[arr[(j)*batch:(j+1)*batch],arr[(j)*batch:(j+1)*batch]] /= K[arr[(j)*batch:(j+1)*batch],arr[(j)*batch:(j+1)*batch]]
        elif div == 'l2':
            Gd = nx.sum(G, 0, keepdims=True) + nx.sum(G, 1, keepdims=True) + 1e-16
            G = G * K / Gd
        elif div == 'l2_2':
...
def mm_unbalanced_revised_screening_fortest(a, b, M, reg_m, l_rate=0.5,screening=None, div='kl', G0=None, numItermax=1000,
    ...
    for i in range(numItermax):
        Gprev = G
        t_screening = screening.update(G.flatten())
...
def mm_unbalanced_revised_screening_for_pic(a, b, M, reg_m, saveround=10000, l_rate=0.5,screening=None, div='kl', G0=None, numItermax=1000,
                  stopThr=1e-15, verbose=False, log=False):
    r"""
    Solve the unbalanced optimal transport problem and return the OT plan.
    The function solves the following optimization problem:
    ...
    for i in range(numItermax):
        Gprev = G
        if(i%saveround==1):
            dis,w=screening.update(G.flatten())
            log ["opt_alg"].append(dis["opt_alg"])
            log ["opt_proj1"].append(dis["opt_proj1"])
            log ["opt_proj2"].append(dis["opt_proj2"])
            log ["alg_proj1"].append(dis["alg_proj1"])
            log ["alg_proj2"].append(dis["alg_proj2"])
            log ["screening_area1"].append(w["screening_area1"])
            log ["screening_area2"].append(w["screening_area2"])
            log ["screening_ps"].append(w["screening_ps"])
            log["screening_p1"].append(w["screening_p1"])
            log["screening_p2"].append(w["screening_p2"])
            if log:
                log['err'].append(err)
                log['G'].append(G)
...
def mm_unbalanced_revised_screening_for_zero(a, b, M, reg_m, saveround=10000, l_rate=0.5,screening=None, div='kl', G0=None, numItermax=1000,
                  stopThr=1e-15, verbose=False, log=False):
    r"""
    Solve the unbalanced optimal transport problem and return the OT plan.
    The function solves the following optimization problem:
...
def sinkhorn_unbalanced_screening(a, b, M, reg, reg_m, method='sinkhorn',screening = None,periter=100, numItermax=1000,
                        stopThr=1e-6, verbose=False, log=False, **kwargs):
    r"""
    Solve the unbalanced entropic regularization optimal transport problem
    and return the OT plan
    ...
    if method.lower() == 'sinkhorn':
        return sinkhorn_knopp_unbalanced_screening(a, b, M, reg, reg_m,
                                        screening = screening,periter=periter,
                                         numItermax=numItermax,
                                         stopThr=stopThr, verbose=verbose,
                                         log=log, **kwargs)
...
def sinkhorn_knopp_unbalanced_screening(a, b, M, reg, reg_m, numItermax=1000,screening = None,
                                periter = 100,
                              stopThr=1e-6, verbose=False, log=False, **kwargs):
    r"""
    Solve the entropic regularization unbalanced optimal transport problem and return the loss
    ...
    for i in range(numItermax):
        if i%periter==0:
            wc = screening.update(u[:, None] * K * v[None, :])
